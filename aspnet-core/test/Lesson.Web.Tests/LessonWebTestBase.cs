...
{
    public abstract class LessonWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static LessonWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(LessonWebMvcModule).Assembly.FullName);
        }
        ...
        protected void UsingDbContext(Action<LessonDbContext> action)
        {
            using (var context = IocManager.Resolve<LessonDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected T UsingDbContext<T>(Func<LessonDbContext, T> func)
        {
            T result;
        ...
        protected async Task UsingDbContextAsync(Func<LessonDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<LessonDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<LessonDbContext, Task<T>> func)
        {
            T result;
